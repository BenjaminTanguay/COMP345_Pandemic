...
void Display::charterFlight()
{
	Player * currentPlayer = Session::getInstance().getPlayers()->at(Session::getInstance().getCurrentPlayer());
	vector<City *> * listOfCities = new vector<City *>;
	int userInput = cityChoice(currentPlayer, "Where would you like to charter a flight to?", listOfCities);
...
void Display::gameInstantiation()
{
	...
	ConsoleFormat::printEmptyLineWall();
	ConsoleFormat::printEmptyLineWall();
	Session::getInstance().setDifficultyLevel(inputCheck(1, 3));
...
void Display::playEventCards()
{
	...
	int cardChoice = inputCheck(1, pairEventCardPlayerIndex->size()) - 1;
	...
	case 1:
		...
		player = players->at(inputCheck(1, players->size()) - 1);
		listOfCities = new vector<City *>;
		
		userInput = cityChoice(player, "Which city would you like to airlift " + player->getPlayerName() + " to?", listOfCities);
	...
	case 3:
		...
		if (discardDeck->size() == 0) {
			...
			userInput = inputCheck(1, discardDeck->size()) - 1;
	...
	case 4:
		...
		do {
			...
			listCities(listOfCities, PlayMap::getInstance()->getOrigin());
			verticalLines += ConsoleFormat::printEmptyLineWall();
			completeBottomWidget(verticalLines);
			userInput = inputCheck(1, listOfCities->size()) - 1;
...
int Display::cityChoice(Player * player, string question, vector<City *> * listOfCities)
{
	...
	completeBottomWidget(verticalLines);
	return inputCheck(1, listOfCities->size()) - 1;
}
